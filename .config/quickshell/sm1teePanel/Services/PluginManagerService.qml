...
Singleton {
    ...
    readonly property bool verboseLogs: Quickshell.env("SM1TEE_VERBOSE_LOGS") === "1"
    ...
    function connectSocket() {
        if (!serverAvailable || isConnected || isConnecting) {
            return
        }
    ...
    Socket {
        ...
        parser: SplitParser {
            onRead: line => {
                ...
                if (root.verboseLogs) {
                    console.log("PluginManagerService: Request socket <<", line)
                }
                ...
                try {
                    ...
                    console.warn("PluginManagerService: Failed to parse request response:", line, e)
                }
            }
        }
    }
    ...
    Socket {
        ...
        parser: SplitParser {
            onRead: line => {
                ...
                if (root.verboseLogs) {
                    console.log("PluginManagerService: Subscribe socket <<", line)
                }
                ...
                try {
                    ...
                    console.warn("PluginManagerService: Failed to parse subscription event:", line, e)
                }
            }
        }
    }
    ...
    function sendSubscribeRequest() {
        ...
        if (verboseLogs) {
            console.log("PluginManagerService: Subscribing to all services")
        }
    ...
    function handleSubscriptionEvent(response) {
        if (response.error) {
            if (response.error.includes("unknown method") && response.error.includes("subscribe")) {
                if (!shownOutdatedError) {
                    console.error("PluginManagerService: Server does not support subscribe method")
        ...
        if (service === "server") {
            ...
            console.log("PluginManagerService: Connected (API v" + apiVersion + ") -", JSON.stringify(capabilities))

            if (apiVersion < expectedApiVersion) {
                ToastService.showError("Server is outdated (API v" + apiVersion + ", expected v" + expectedApiVersion + ")")
            }
