...
Singleton {
    ...
    Process {
        ...
        command: ["bash", "-c", "ddcutil detect --brief 2>/dev/null | grep -E '^Display[[:space:]]+[0-9]' | awk '{print \"{\\\"display\\\":\" $2 \",\\\"name\\\":\\\"ddc-\" $2 \"\\\",\\\"class\\\":\\\"ddc\\\"}\"}' | tr '\\n' ',' | sed 's/,$//' | sed 's/^/[/' | sed 's/$/]/' || echo '[]'"]
        running: false
            ...
            onStreamFinished: {
                ...
                for (const line of lines) {
                    ...
                    if (parts.length >= 5) {
                        ...
                        // Only keep backlight class devices (actual displays)
                        // Filter out leds class (keyboard LEDs, network LEDs, etc)
                ...
                console.log("DisplayService: Found", newDevices.length, "backlight devices from brightnessctl")
