...
Singleton {
    ...
    function runUpdates() {
        ...
        // Улучшенная команда с обработкой ошибок и уведомлением
        const updateCommand = `
            ${sudo} ${pkgManager} ${params}
            EXIT_CODE=$?
            if [ $EXIT_CODE -eq 0 ]; then
                echo ""
                echo "✓ Обновления успешно установлены!"
                notify-send "Обновления системы" "Все обновления успешно установлены" -i system-software-update
            else
                echo ""
                echo "✗ Ошибка при установке обновлений (код: $EXIT_CODE)"
                notify-send "Обновления системы" "Ошибка при установке обновлений" -u critical -i dialog-error
            fi
            echo ""
