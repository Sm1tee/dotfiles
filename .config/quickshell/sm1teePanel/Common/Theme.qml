...
Singleton {
    ...
    property color surface: {
        if (typeof SettingsData !== "undefined" && SettingsData.surfaceBase === "s") {
            // В темной теме: делаем темнее (background темнее чем surface)
            // В светлой теме: делаем темнее (surfaceContainer темнее чем surface)
            return isLightMode ? currentThemeData.surfaceContainer : currentThemeData.background
        }
    ...
    property color surfaceContainer: {
        if (typeof SettingsData !== "undefined" && SettingsData.surfaceBase === "s") {
            // В темной теме: делаем темнее (surface темнее чем surfaceContainer)
            // В светлой теме: делаем темнее (surfaceContainerHigh темнее чем surfaceContainer)
            return isLightMode ? currentThemeData.surfaceContainerHigh : currentThemeData.surface
        }
        return currentThemeData.surfaceContainer
    }
    property color surfaceContainerHigh: {
        if (typeof SettingsData !== "undefined" && SettingsData.surfaceBase === "s") {
            // В темной теме: делаем темнее (surfaceContainer темнее чем surfaceContainerHigh)
            // В светлой теме: делаем темнее (surfaceContainerHighest темнее чем surfaceContainerHigh)
            return isLightMode ? currentThemeData.surfaceContainerHighest : currentThemeData.surfaceContainer
        }
        return currentThemeData.surfaceContainerHigh
    }
    property color surfaceContainerHighest: {
        if (typeof SettingsData !== "undefined" && SettingsData.surfaceBase === "s") {
            // В темной теме: делаем темнее (surfaceContainerHigh темнее чем surfaceContainerHighest)
            // В светлой теме: уже самый темный, оставляем как есть
            return isLightMode ? currentThemeData.surfaceContainerHighest : currentThemeData.surfaceContainerHigh
        }
    ...
    function getPowerProfileDescription(profile) {
        switch (profile) {
        case PowerProfile.PowerSaver:
            return "Продлить время работы батареи"
        case PowerProfile.Balanced:
            return "Баланс энергии и производительности"
        case PowerProfile.Performance:
            return "Приоритет производительности"
        default:
            return "Пользовательский профиль питания"
        }
    }
