...
Singleton {
    ...
    // Хранилище для текущих mountPath виджетов дисков (только в памяти)
    property var diskWidgetMountPaths: ({})
    ...
    function parseSettings(content) {
        try {
            if (content && content.trim()) {
                ...
                pinnedTopApps = settings.pinnedTopApps !== undefined ? settings.pinnedTopApps : []
                hiddenApps = settings.hiddenApps !== undefined ? settings.hiddenApps : []
    ...
    function addPinnedTopApp(appId) {
        if (!appId) return
        var current = [...pinnedTopApps]
        if (current.indexOf(appId) === -1) {
            current.push(appId)
            pinnedTopApps = current
            saveSettings()
        }
    }

    function removePinnedTopApp(appId) {
        if (!appId) return
        pinnedTopApps = pinnedTopApps.filter(id => id !== appId)
        saveSettings()
    }

    function isPinnedTopApp(appId) {
        return appId && pinnedTopApps.indexOf(appId) !== -1
    }

    function addHiddenApp(appId) {
        if (!appId) return
        var current = [...hiddenApps]
        if (current.indexOf(appId) === -1) {
            current.push(appId)
            hiddenApps = current
            saveSettings()
    ...
    function removeHiddenApp(appId) {
        if (!appId) return
        hiddenApps = hiddenApps.filter(id => id !== appId)
        saveSettings()
    }

    function isHiddenApp(appId) {
    ...
    FileView {
        ...
        path: isGreeterMode ? "" : StandardPaths.writableLocation(StandardPaths.GenericStateLocation) + "/sm1teePanel/session.json"
    ...
    Process {
        ...
        command: ["sh", "-c", "CONFIG_DIR=\"" + _stateDir
            + "/sm1teePanel\"; if [ -f \"$CONFIG_DIR/default-session.json\" ] && [ ! -f \"$CONFIG_DIR/session.json\" ]; then cp --no-preserve=mode \"$CONFIG_DIR/default-session.json\" \"$CONFIG_DIR/session.json\" && echo 'copied'; else echo 'not_found'; fi"]
        running: false
        onExited: exitCode => {
            if (exitCode === 0) {
                settingsFile.reload()
