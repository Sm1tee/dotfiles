...
Item {
    ...
    Component {
        ...
        DiskUsageDetail {
            currentMountPath: {
                const instanceId = root.expandedWidgetData?.instanceId || ""
                // Читаем из SessionData если есть, иначе из настроек
                return SessionData.diskWidgetMountPaths[instanceId] || root.expandedWidgetData?.mountPath || "/"
            }
