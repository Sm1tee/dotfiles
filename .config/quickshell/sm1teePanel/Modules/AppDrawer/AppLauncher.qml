...
Item {
    ...
    property string selectedCategory: "Все"
    ...
    readonly property var categories: {
        const allCategories = AppSearchService.getAllCategories().filter(cat => cat !== "Образование" && cat !== "Наука")
        const result = ["Все"]
        return result.concat(allCategories.filter(cat => cat !== "Все"))
    }
    ...
    property var _watchPinnedTopApps: SessionData.pinnedTopApps
    property var _watchHiddenApps: SessionData.hiddenApps
    ...
    function updateFilteredModel() {
        ...
        // Фильтруем приложения в зависимости от режима
        apps = apps.filter(app => {
            const appId = app.id || app.execString || app.exec || ""
            const isHidden = SessionData.isHiddenApp(appId)
            // Если режим "показать скрытые" - показываем ТОЛЬКО скрытые
            // Иначе показываем только НЕ скрытые
        ...
        if (searchQuery.length === 0) {
            apps = apps.sort((a, b) => {
                                 ...
                                 // Закрепленные приложения всегда вверху
                                 const aPinned = SessionData.isPinnedTopApp(aId)
                                 const bPinned = SessionData.isPinnedTopApp(bId)
                                 if (aPinned !== bPinned) {
                                     return bPinned ? 1 : -1
                                 }
                                 
                                 // Если оба закреплены, сортируем по порядку закрепления
                                 if (aPinned && bPinned) {
                                     const aIndex = SessionData.pinnedTopApps.indexOf(aId)
                                     const bIndex = SessionData.pinnedTopApps.indexOf(bId)
    ...
    on_WatchPinnedTopAppsChanged: updateFilteredModel()
    on_WatchHiddenAppsChanged: updateFilteredModel()
    onShowHiddenChanged: updateFilteredModel()
