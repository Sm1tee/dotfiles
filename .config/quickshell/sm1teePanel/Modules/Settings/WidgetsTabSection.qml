                    ...
                    Row {
                                ...
                                currentValue: {
                                    ...
                                    if (DgopService.diskMounts && DgopService.diskMounts.length > 0) {
                                        for (let i = 0; i < DgopService.diskMounts.length; i++) {
                                            if (DgopService.diskMounts[i].mount === mountPath) {
                                                return DgopService.diskMounts[i].displayName || mountPath
                                            }
                                        }
                                    }
                                    return mountPath === "/" ? "System" : mountPath
                                }
                                options: {
                                    if (!DgopService.diskMounts || DgopService.diskMounts.length === 0) {
                                        return ["System"]
                                    }
                                    return DgopService.diskMounts.map(mount => mount.displayName || mount.mount)
                                }
                                onValueChanged: value => {
                                    let mountPath = value
                                    if (DgopService.diskMounts && DgopService.diskMounts.length > 0) {
                                        for (let i = 0; i < DgopService.diskMounts.length; i++) {
                                            if (DgopService.diskMounts[i].displayName === value) {
                                                mountPath = DgopService.diskMounts[i].mount
                                                break
                                            }
                                        }
                                    }
                                    root.diskMountSelectionChanged(root.sectionId, index, mountPath)
                                }
                            }
                        }
                        ...
                        DankActionButton {
                            ...
                            onEntered: {
                                ...
                                if (minimumWidthTooltipLoader.item) {
                                    var currentEnabled = modelData.minimumWidth !== undefined ? modelData.minimumWidth : true
                                    const tooltipText = currentEnabled ? "Фиксированная ширина" : "Динамическая ширина"
                        ...
                        Row {
                            ...
                            DankActionButton {
                                ...
                                onEntered: {
                                    dateOrderTooltipLoader.active = true
                                    if (dateOrderTooltipLoader.item) {
                                        const tooltipText = SettingsData.clockDateFirst ? "Время • Дата" : "Дата • Время"
                                        const p = dateOrderButton.mapToItem(null, dateOrderButton.width / 2, 0)
                                        dateOrderTooltipLoader.item.show(tooltipText, p.x, p.y - 40, null)
                                    }
                                }
                                onExited: {
                                    if (dateOrderTooltipLoader.item) {
                                        dateOrderTooltipLoader.item.hide()
                                    }
                                    dateOrderTooltipLoader.active = false
                                }
                            }
                        ...
                        DankActionButton {
                            ...
                            onClicked: {
                                ...
                                keyboardLayoutContextMenu.sectionId = root.sectionId
                                keyboardLayoutContextMenu.widgetIndex = index
                                var parentPos = parent.mapToItem(root, 0, 0)
                                keyboardLayoutContextMenu.x = parentPos.x - 210
                                keyboardLayoutContextMenu.y = parentPos.y - 10
                                keyboardLayoutContextMenu.open()
                            }
                        }
                        ...
                        DankActionButton {
                            ...
                            onEntered: {
                                ...
                                if (visibilityTooltipLoader.item) {
                                    const tooltipText = modelData.enabled ? "Скрыть" : "Показать"
            ...
            Column {
                ...
                Rectangle {
                    ...
                    DankToggle {
                        ...
                        width: 40
                        height: 20
                        checked: SettingsData.keyboardLayoutShowFlag
                        onToggled: {
                            SettingsData.setKeyboardLayoutShowFlag(toggled)
                        }
                    }
                    ...
                    MouseArea {
                        ...
                        onPressed: {
                            ...
                            SettingsData.setKeyboardLayoutShowFlag(flagToggle.checked)
                        }
                    }
                }
            }
