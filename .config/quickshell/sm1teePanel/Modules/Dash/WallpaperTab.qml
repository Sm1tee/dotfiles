...
import Quickshell
import qs.Common
import qs.Modals.FileBrowser as FB
import qs.Widgets
...
Item {
    ...
    function setInitialSelection() {
        ...
        if (!currentWallpaper || wallpaperFolderModel.count === 0) {
            ...
            wallpaperGrid.positionViewAtIndex(0, GridView.Contain)
            updateSelectedFileName()
            Qt.callLater(() => { Qt.callLater(() => { enableAnimation = true }) })
            return
        }
        ...
        for (var i = 0; i < wallpaperFolderModel.count; i++) {
            ...
            if (filePath && filePath.toString().replace(/^file:\/\//, '') === currentWallpaper) {
                ...
                wallpaperGrid.currentIndex = targetIndex
                wallpaperGrid.positionViewAtIndex(targetIndex, GridView.Contain)
                updateSelectedFileName()
                Qt.callLater(() => { Qt.callLater(() => { enableAnimation = true }) })
                return
            }
    ...
    function loadWallpaperDirectory() {
        ...
        if (!currentWallpaper || currentWallpaper.startsWith("#")) {
            if (SessionData.wallpaperLastPath && SessionData.wallpaperLastPath !== "") {
                wallpaperDir = SessionData.wallpaperLastPath
            } else {
                wallpaperDir = ""
            }
            return
    ...
    FB.FileBrowserModal {
        ...
        browserTitle: "Выбрать папку с обоями"
        ...
        onFileSelected: path => {
            ...
            if (dirPath) {
                ...
                SessionData.setWallpaperLastPath(dirPath)
            }
            close()
        }
    }
    ...
    Column {
        ...
        Item {
            ...
            GridView {
                ...
                delegate: Item {
                    ...
                    Rectangle {
                        ...
                        Rectangle {
                            anchors.fill: parent
                            color: wallpaperMouseArea.containsMouse ? Qt.rgba(Theme.primary.r, Theme.primary.g, Theme.primary.b, 0.2) : "transparent"
                            radius: parent.radius
                        }
            ...
            Column {
                ...
                StyledText {
                    text: "Нажмите на иконку папки чтобы выбрать"
        ...
        Column {
            ...
            Row {
                ...
                Row {
                    ...
                    StyledText {
                        anchors.verticalCenter: parent.verticalCenter
                        text: wallpaperFolderModel.count > 0 ? `${wallpaperFolderModel.count} обоев  •  ${currentPage + 1} / ${totalPages}` : "Нет обоев"
                        font.pixelSize: Theme.fontSizeMedium
