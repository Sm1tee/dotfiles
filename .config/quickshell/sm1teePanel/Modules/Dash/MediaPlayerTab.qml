...
Item {
    ...
    readonly property bool isRightEdge: SettingsData.barPosition === SettingsData.Position.Right
    ...
    function loadArtwork(url) {
        ...
        if (url.startsWith("http://") || url.startsWith("https://")) {
            const filename = "/tmp/.sm1tee/trackart_" + Date.now() + ".jpg"
            activeTrackArtFile = filename

            cleanupProcess.command = ["sh", "-c", "mkdir -p /tmp/.sm1tee && find /tmp/.sm1tee -name 'trackart_*' ! -name '" + filename.split('/').pop() + "' -delete"]
    ...
    Item {
        ...
        Popup {
            ...
            Column {
                ...
                StyledText {
                    text: "Устройства вывода звука (" + audioDevicesDropdown.availableDevices.length + ")"
        ...
        Popup {
            ...
            Column {
                ...
                StyledText {
                    text: "Медиа плееры (" + (allPlayers?.length || 0) + ")"
                    font.pixelSize: Theme.fontSizeMedium
                    font.weight: Font.Medium
                    color: Theme.surfaceText
                    width: parent.width
