...
lock='🔒'     # Или 🔐, 🔑, ⌿
logout='🚪'   # Или →, ⇥, ⎋, X, 🔚
yes='Да'      # Используем текст для подтверждения
no='Нет'
...
case ${chosen} in
    ...
    $reboot)
        ...
        ;;
    $lock)
		# Проверяем, существует ли команда hyprlock перед выполнением
		if command -v hyprlock >/dev/null; then
			hyprlock
		else
			# Можно вывести уведомление об ошибке, если hyprlock не найден
